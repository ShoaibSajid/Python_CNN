...
if Dataset=="MNIST":
  ...
  pool0   = MaxPool2              (                                 )     # 28x28x8   -> 14x14x8  (MaxPooling 2x2)
  conv1   = Conv3x3_n_to_n_padding( output=16     ,   input=8       )     # 14x14x8   -> 14x14x16 (Convolution with 8 filters)
  pool1   = MaxPool2              (                                 )     # 28x28x8   -> 14x14x8  (MaxPooling 2x2)
  conv2   = Conv3x3_n_to_n_padding( output=16     ,   input=16      )
  pool2   = MaxPool2              (                                 )     # 14x14x16  -> 07x07x16 (MaxPooling 2x2)
  conv3   = Conv3x3_n_to_n_padding( output=32     ,   input=16      )
  FC_size= 4 * 4 * 32
  fc0     = FC                    (  FC_size      ,   FC_size       )     # 784       -> 784      (FC)
  fc1     = FC                    (  FC_size      ,   10            )     # 784       -> 10       (FC)
  softmax = Softmax               (                                 )     # 14x14x8   -> 10       (Softmax)
...
def forward(im, label, debug=False):
  ...
  if Dataset=="MNIST":
    ...
    # out_pool3 = pool3.forward   ( out_conv3     )
    # Swap axes to realign for flattening
    out_swap0 = np.swapaxes     ( out_conv3,0,2 )
    out_swap1 = np.swapaxes     ( out_swap0,1,2 )
    # FC0 and Relu
    out_fc0   = fc0.forward     ( out_swap1     )
    # FC1 and Relu
    out_fc1   = fc1.forward     ( out_fc0       )
    # SoftMax
    out_soft  = softmax.forward ( out_fc1       )
...
def backward(label, out, loss=1, lr=0.005):
  ...
  # Conv 3 with Pool 
  gradient_conv3 = conv3.backprop     ( gradient_swap1     ,              lr  )
  # Conv 2  
  gradient_pool2 = pool2.backprop     ( gradient_conv3                        ) 
  gradient_conv2 = conv2.backprop     ( gradient_pool2     ,              lr  )
  # Conv 1 
  gradient_pool1 = pool1.backprop     ( gradient_conv2                        ) 
...
def adjust_lr(acc, lr=.005):
  if   acc > 98-30: lr=0.00001
  elif acc > 95-30: lr=0.0005
  elif acc > 90-30: lr=0.001
  elif acc > 80-30: lr=0.002
  elif acc > 70-30: lr=0.003
  elif acc > 60-30: lr=0.004
  return lr
    ...
    for i, (im, label) in tqdm(enumerate(zip(train_images, train_labels))):
      ...
      if i % training_acc_internal == training_acc_internal-1:
        ...
        if i+1-training_acc_internal == 0:
          ...
          max_acc = training_acc
          save_weights(f'weights/4Conv_{Dataset}_best_{training_acc}.pkl', lr, max_acc)
          save_weights(f'weights/4Conv_{Dataset}_last.pkl', lr, max_acc)
