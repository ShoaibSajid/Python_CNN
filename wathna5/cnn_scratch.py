...
class SpatialBatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_params):
    ...
    pre_m_normolized, cache= BatchNorm.forward(pre_m, gamma, beta, bn_params)
...
class Conv_BatchNorm_ReLU(object):
  ...
  def forward(x, w, gamma, beta, conv_param, bn_params):
    a, conv_cache = Conv.forward(x, w, conv_param)
    an, bn_cache = SpatialBatchNorm.forward(a, gamma, beta, bn_params)
...
class DeepConvNet(object):
  ...
  def loss(self, X, gt_boxes=None, gt_classes=None, num_boxes=None):
    ...
    for i in range(self.num_layers-1):
      ...
      if i in self.max_pools:
        ...
        if self.slowpool and i == 6 :
          ...
          out, cache['{}'.format(i)] = MaxPool.forward(out, slowpool_param)
        if self.batchnorm:
    ...
    # return loss, dout
    last_dout, dw, db  = ConvB.backward(dout, cache['{}'.format(i)])

    # print("last dout", last_dout)
    ...
    # with open('./grad_scratch/conv_param/w9', mode='w') as f:
    #   f.write(str(dw))
    # with open('./grad_scratch/bias_param/b9', mode='w') as f:
    #   f.write(str(db))
    # print(dw, db)
