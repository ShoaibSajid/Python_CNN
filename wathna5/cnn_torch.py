...
class DeepConvNetTorch_2(object): # Python based CNN Implementation
  """
  ...
  def train(self, X, gt_boxes=None, gt_classes=None, num_boxes=None):
    ...
    if True:
      ...
      Conv5 , cache['61'] = Padd.forward                    (Conv5 , pad_param)
      Conv6 , cache['60'] = FastMaxPool.forward             (Conv5 , slowpool_param)
    ...
    if True: 
      ...
      # dout60                                              = FastMaxPool.backward             (dout7, cache['60'])
      # dout61                                              = Padd.backward                    (dout60, cache['61'])
      # dout5, grads['W5'], grads['gamma5'], grads['beta5'] = Conv_BatchNorm_ReLU.backward     (dout60, cache['5'])
      ...
      return bbox_pred, conf_pred, class_pred
...
class DeepConvNetTorch(object):
  ...
  def loss(self, X, gt_boxes=None, gt_classes=None, num_boxes=None):
    """
    Evaluate loss and gradient for the deep convolutional network.
    Input / output: Same API as ThreeLayerConvNet.
    """
    ...
    # print(dout.dtype, dout[dout!=0])
    # loss.backward(retain_graph=True)
    # print(out.grad)
    # print(dout)
    print(dout.dtype, dout[dout!=0])
    # print(model_ll.conv9.weight.grad)
    # print(model_ll.conv9.bias.grad)
    
    # box_loss.retain_grad()
    # iou_loss.retain_grad()
    ...
    # return loss, dout
    last_dout, dw, db  = FastConvWB.backward(dout, cache['{}'.format(i)])
    # grads['W{}'.format(i)] = model_ll.conv9.weight.grad
    # grads['b{}'.format(i)] = model_ll.conv9.bias.grad
    grads['W{}'.format(i)] = dw
    print(f'dw{i}', dw.shape, dw[dw!=0], dw.dtype)
    grads['b{}'.format(i)] = db
    print(f'db{i}', db.shape, db[db!=0], db.dtype)
    # with open('./grad_torch/conv_param/w9', mode='w') as f:
    #   f.write(str(dw))
    # with open('./grad_torch/bias_param/b9', mode='w') as f:
    #   f.write(str(db))
    # print(dw, db)
    ...
    for i in range(i-1,-1,-1):
      if i in self.max_pools:
        if self.batchnorm:
          ...
          grads['beta{}'.format(i)] = dbetta1
          grads['gamma{}'.format(i)] = dgamma1
          print(f'dw{i}', dw.shape, dw[dw!=0])
        else:  
          last_dout, dw  = Conv_ReLU_Pool.backward(last_dout, cache['{}'.format(i)])
          grads['W{}'.format(i)] = dw
          print(f'dw{i}', dw.shape)
      else:
        if self.batchnorm:
          last_dout, dw, dgamma1,dbetta1  = Conv_BatchNorm_ReLU.backward(last_dout, cache['{}'.format(i)])
          grads['W{}'.format(i)] = dw
