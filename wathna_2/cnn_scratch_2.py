...
from yolov2 import Yolov2_lastlayer
...
class DeepConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32),
    ...
    self.yolo_last_layer_model = Yolov2_lastlayer()
    self.yolo_last_layer_model.train()
    ...
    if not self.batchnorm:
      ...
      params_per_macro_layer = 5  # weight, bias, scale, shift
  ...
  def loss(self, X, y=None):
    ...
    # Forward Propagation
    _out['0'],cache[0]  = Conv_BatchNorm_ReLU_Pool.forward(_input       , self.params['W0'], self.params['gamma0'],self.params['beta0'],conv_param,self.bn_params[0],pool_param)
    _out['1'],cache[1]  = Conv_BatchNorm_ReLU_Pool.forward(_out['0']    , self.params['W1'], self.params['gamma1'],self.params['beta1'],conv_param,self.bn_params[1],pool_param)
    _out['2'],cache[2]  = Conv_BatchNorm_ReLU_Pool.forward(_out['1']    , self.params['W2'], self.params['gamma2'],self.params['beta2'],conv_param,self.bn_params[2],pool_param)
    _out['3'],cache[3]  = Conv_BatchNorm_ReLU_Pool.forward(_out['2']    , self.params['W3'], self.params['gamma3'],self.params['beta3'],conv_param,self.bn_params[3],pool_param)
    _out['4'],cache[4]  = Conv_BatchNorm_ReLU_Pool.forward(_out['3']    , self.params['W4'], self.params['gamma4'],self.params['beta4'],conv_param,self.bn_params[4],pool_param)
    _out['5'],cache[5]  = Conv_BatchNorm_ReLU.forward     (_out['4']    , self.params['W5'], self.params['gamma5'],self.params['beta5'],conv_param,self.bn_params[5]) 
    _out['5_0']         = F.pad                           (_out['5']    , (0, 1, 0, 1))
    _out['5_1'], _      = MaxPool.forward                 (_out['5_0']  , slowpool_param)
    _out['6'],cache[6]  = Conv_BatchNorm_ReLU.forward     (_out['5_1']  , self.params['W6'], self.params['gamma6'],self.params['beta6'],conv_param,self.bn_params[6]) 
    _out['7'],cache[7]  = Conv_BatchNorm_ReLU.forward     (_out['6']    , self.params['W7'], self.params['gamma7'],self.params['beta7'],conv_param,self.bn_params[7]) 
    
    # conv_param['pad']   = 0 
    # final_out,cache[8]    = ConvB.forward            (_out['7']  , self.params['W8'], self.params['b8'], conv_param)
    
    __final_out, losses = self.yolo_last_layer_model     (_out['7']  , gt_boxes, gt_classes, num_boxes)
    
    losses.backward()
    optimizer.step()
    
    dw = self.yolo_last_layer_model.weight.grad()
    db = self.yolo_last_layer_model.bias.grad()
    
    Conv_BatchNorm_ReLU.backward(dw, db )
    ...
    # last_dout, dw, db  = Conv.backward(dout, cache['{}'.format(i)])
