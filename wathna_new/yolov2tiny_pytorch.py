...
class Yolov2(nn.Module):
    ...
    def forward(self, x, gt_boxes=None, gt_classes=None, num_boxes=None, training=False):
        ...
        """
        i = 1
        with open('./parameter_torch/layer{}_input()'.format(i), mode='w') as f:
            f.write(str(x))
        x = self.maxpool(self.lrelu(self.bn1(self.conv1(x))))
        with open('./parameter_torch/layer{}(conv_bn_relu_pool)'.format(i), mode='w') as f:
            f.write(str(x))
        i += 1
        ...
        with open('./parameter_torch/layer{}(conv_bn_relu)'.format(i), mode='w') as f:
            f.write(str(x))
        x = F.pad(x, (0, 1, 0, 1))
        x = self.slowpool(x)
        with open('./parameter_torch/layer{}(after slowpool)'.format(i), mode='w') as f:
            f.write(str(x))
        i += 1
